...
const missing = targets.filter(target => !fs.existsSync(path.join(root, 'prebuilds', target, 'node.napi.node')))

if (missing.length > 0) {
  console.error('infinitysqlite: faltan prebuilds antes de publicar:')
  missing.forEach(target => console.error(`  - prebuilds/${target}/node.napi.node`))

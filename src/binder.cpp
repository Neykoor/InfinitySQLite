...
void AggregateStep(sqlite3_context *ctx, int argc, sqlite3_value **argv) {
  ...
  if (!state->initialized) {
    ...
    state->errored = false;
    state->accumulator = new Napi::Reference<Napi::Value>(Napi::Persistent(data->startValue.Value()));
  }
  if (state->errored) {
    return;
  }
  ...
  std::vector<napi_value> args;
  args.reserve(argc + 1);
  args.push_back(state->accumulator->Value());
  for (int i = 0; i < argc; i++) {
    args.push_back(SqliteValueToNapi(env, argv[i]));
  }
  ...
  state->accumulator = new Napi::Reference<Napi::Value>(Napi::Persistent(nextAcc));
}
...
void AggregateFinal(sqlite3_context *ctx) {
  ...
  Napi::Value acc = (state && state->initialized) ? state->accumulator->Value() : data->startValue.Value();

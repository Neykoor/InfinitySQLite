...
Napi::Function InfinityDatabase::GetClass(Napi::Env env) {
  return DefineClass(env, "InfinityDatabase", {
    ...
    InstanceMethod("registerAggregate", &InfinityDatabase::RegisterAggregate),
    InstanceMethod("checkpoint", &InfinityDatabase::Checkpoint),
    InstanceMethod("backup", &InfinityDatabase::Backup),
    InstanceMethod("serialize", &InfinityDatabase::Serialize),
    InstanceMethod("deserialize", &InfinityDatabase::Deserialize),
...
Napi::Value InfinityDatabase::RegisterAggregate(const Napi::CallbackInfo &info) {
  Napi::Env env = info.Env();
  if (!open_ || !handle_) {
    Napi::Error::New(env, "Database connection is not open").ThrowAsJavaScriptException();
    return env.Undefined();
  }
  if (info.Length() < 6 || !info[0].IsString() || !info[1].IsFunction() || !info[2].IsNumber() || !info[3].IsBoolean()) {
    Napi::TypeError::New(env, "Expected (name, stepFn, nArg, deterministic, resultFn, start)").ThrowAsJavaScriptException();
    return env.Undefined();
  }
  std::string name = info[0].As<Napi::String>().Utf8Value();
  Napi::Function stepFn = info[1].As<Napi::Function>();
  int nArg = info[2].As<Napi::Number>().Int32Value();
  bool deterministic = info[3].As<Napi::Boolean>().Value();
  Napi::Value startValue = info[5];

  auto *data = new InfinityBinder::AggregateUserData(env, startValue, stepFn);
  if (info[4].IsFunction()) {
    data->resultFn = Napi::Persistent(info[4].As<Napi::Function>());
    data->hasResultFn = true;
  }
  ...
  int rc = sqlite3_create_function_v2(
    handle_, name.c_str(), nArg, flags, data,
    nullptr, InfinityBinder::AggregateStep, InfinityBinder::AggregateFinal,
    InfinityBinder::DestroyAggregateUserData
  );
...
Napi::Value InfinityDatabase::Serialize(const Napi::CallbackInfo &info) {
  Napi::Env env = info.Env();
  if (!open_ || !handle_) {
    Napi::Error::New(env, "Database connection is not open").ThrowAsJavaScriptException();
    return env.Undefined();
  ...
  unsigned char *data = sqlite3_serialize(handle_, "main", &size, 0);
  if (!data) {
    if (size == 0) {
      return Napi::Buffer<uint8_t>::New(env, 0);
    }
    Napi::Error::New(env, "Failed to serialize database").ThrowAsJavaScriptException();
    return env.Undefined();
  }
  Napi::Buffer<uint8_t> buffer = Napi::Buffer<uint8_t>::Copy(env, data, static_cast<size_t>(size));
  sqlite3_free(data);
  return buffer;
}

Napi::Value InfinityDatabase::Deserialize(const Napi::CallbackInfo &info) {
  Napi::Env env = info.Env();
  if (!open_ || !handle_) {
    Napi::Error::New(env, "Database connection is not open").ThrowAsJavaScriptException();
    return env.Undefined();
  ...
  if (info.Length() < 1 || !info[0].IsBuffer()) {
    Napi::TypeError::New(env, "Expected a Buffer to deserialize").ThrowAsJavaScriptException();
    return env.Undefined();
  }
  Napi::Buffer<uint8_t> input = info[0].As<Napi::Buffer<uint8_t>>();
  size_t length = input.Length();

  unsigned char *copy = static_cast<unsigned char *>(sqlite3_malloc64(length > 0 ? length : 1));
  if (!copy) {
    Napi::Error::New(env, "Failed to allocate memory for deserialize").ThrowAsJavaScriptException();
    return env.Undefined();
  }
  if (length > 0) {
    std::memcpy(copy, input.Data(), length);
  }

  int rc = sqlite3_deserialize(
    handle_, "main", copy, static_cast<sqlite3_int64>(length), static_cast<sqlite3_int64>(length),
